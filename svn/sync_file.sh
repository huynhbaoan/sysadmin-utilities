...
while read LINE
do
    rsync -avzp -e "ssh -i $IDENTITY" --remove-source-files /svnbackup/$LINE gsysadmin@192.168.38.130:/git/svnbackup
    ssh -n -i $IDENTITY gsysadmin@192.168.38.130 "/git/svnbackup/s3_upload.sh"
    rsync -avzp -e "ssh -i $IDENTITY" gsysadmin@192.168.38.130:/git/svnbackup/s3_uploaded.list $WORKING_DIR
